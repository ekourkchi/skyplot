...
col_pallet = ['darkviolet', 'blue', 'deepskyblue', 'forestgreen', 'y', 'gold',
              'darkorange', 'red', 'magenta', 'maroon', 'sienna', 'slategrey', 'black']
vel_pallet = [-100000, 0, 250, 500, 750, 1000, 1250,
              1500, 1750, 2000, 2500, 3000, 3500, 100000]
...
class GroupBrowser(object):
    ...
    def __init__(self, A, B, table_gal, A_cluster, B_cluster, table_all, root_win, mode):
        ...
        for p in range(len(self.id_gal)):
            ...
            if Dist < 1:
                Dist = 1.
            if self.mDist_gal[p] != 0:
                Dist = self.mDist_gal[p]
            ...
            for q in range(0, len(u)):
                ...
                X[q] = x
                Y[q] = y
            line, = plt.plot(X, Y, '-.', markersize=1,
                             color='red', picker=False, visible=False)
            line.set_dashes([8, 3])
            self.listplot_gal.append([self.id_gal[p], line])

        N = len(id)
        NoGroups = len(id[np.where(flag == 5)])
        i = 0
        if NoGroups != 0:
            while flag[i] != 5:
                i += 1
        ...
        while gr == 5:
            ...
            X = []
            Y = []
            while i < N and (flag[i] == 4 or flag[i] == 3):
                RA0 = self.ra[i]
                DEC0 = self.dec[i]
                X.append(RA0)
                Y.append(DEC0)
                ...
                i += 1
            group_plt = plt.plot(
                X, Y, 'o', markersize=3, mfc='none', markeredgecolor='red', visible=False)
...
class PointBrowser(object):
    ...
    def __init__(self, A, B, table_gal, A_cluster, B_cluster, table_all, root_win, mode):
        ...
        self.group_plot = GroupBrowser(
            A, B, table_gal, A_cluster, B_cluster, table_all, root_win, mode)
        ...
        self.r1t_lum_gal = table_gal['r1t_lum']
        self.dist_gal = table_gal['dist']
        ...
        if mode == 'cartesian':
            self.Usgx = self.ax.annotate(
                " ", (0.016, 0.53), xycoords='figure fraction', size=11)
            self.Usgy = self.ax.annotate(
                " ", (0.016, 0.48), xycoords='figure fraction', size=11)
            self.Usgz = self.ax.annotate(
                " ", (0.016, 0.43), xycoords='figure fraction', size=11)
    ...
    def onpick(self, event):
        ...
        if event.mouseevent.button == 1:   # It's just active at left mouse click
            ...
            if N > 1:
                ind = event.ind
                print("\n\n*****************************************************")
                print("***** All Neighbour Objects ************************")
                print("*****************************************************")

                myTable = Table()
                myTable.add_column(Column(data=self.id_gal[ind], name='pgc'))
                myTable.add_column(
                    Column(data=self.flag_gal[ind], name='flag'))
                ...
                myTable.add_column(
                    Column(data=self.r1t_lum_gal[ind],  name='r1t'))
                myTable.add_column(
                    Column(data=self.dist_gal[ind],  name='dist'))
                myTable.add_column(
                    Column(data=self.mDist_gal[ind],  name='dcf#'))
                myTable.add_column(
                    Column(data=self.mDistErr_gal[ind],  name='ed'))
                myTable.add_column(
    ...
    def single_info(self, dataind):
        ...
        if self.flag_gal[ind] == 0 or self.flag_gal[ind] == 3:
            print("\n\n------ Selected Galaxy -------------------------------")
        if self.flag_gal[ind] == 2 or self.flag_gal[ind] == 4:
            print("\n\n------ Selected Group -------------------------------")
        ...
        myTable.add_column(Column(data=self.Ks_gal[ind],  name='Ks'))
        myTable.add_column(Column(data=self.Vls_gal[ind],  name='Vls'))
        myTable.add_column(Column(data=self.r1t_lum_gal[ind],  name='r1t'))
        myTable.add_column(Column(data=self.dist_gal[ind],  name='dist'))
        myTable.add_column(Column(data=self.mDist_gal[ind],  name='dcf#'))
        myTable.add_column(Column(data=self.mDistErr_gal[ind],  name='ed'))
        ...
        if self.mode == 'cartesian':
            sgx, sgy, sgz = xyz_list(self.sgl_gal[ind], self.sgb_gal[ind], self.Vls_gal[ind],
                                     self.dist_gal[ind], self.mDist_gal[ind], self.flag_gal[ind])
...
def table_deliver(filee):
    ...
    try:
        ...
        print("You can use \"python " + \
            sys.argv[0]+" -h\" for help ... (or 'skyplot2 -h') \n", file=sys.stderr)
        exit(1)
...
def file_deliver(filee):
    ...
    if Have_north and not Have_south:
        ...
        id = np.concatenate(
            (north_table['pgc'][north_ind], south_table['pgc'][south_ind]))
        flag = np.concatenate(
            (north_table['flag'][north_ind], south_table['flag'][south_ind]))
        sgl = np.concatenate(
            (north_table['sgl'][north_ind], south_table['sgl'][south_ind]))
        sgb = np.concatenate(
            (north_table['sgb'][north_ind], south_table['sgb'][south_ind]))
        gl = np.concatenate(
            (north_table['gl'][north_ind], south_table['gl'][south_ind]))
        gb = np.concatenate(
            (north_table['gb'][north_ind], south_table['gb'][south_ind]))
        ra = np.concatenate(
            (north_table['ra'][north_ind], south_table['ra'][south_ind]))
        dec = np.concatenate(
            (north_table['dec'][north_ind], south_table['dec'][south_ind]))
        Ks = np.concatenate(
            (north_table['Ks'][north_ind], south_table['Ks'][south_ind]))
        Vls = np.concatenate(
            (north_table['Vls'][north_ind], south_table['Vls'][south_ind]))
        R_2t2 = np.concatenate(
            (north_table['R2t_lum'][north_ind], south_table['R2t_lum'][south_ind]))
        nest = np.concatenate(
            (north_table['nest'][north_ind], south_table['nest'][south_ind]))

        r1t_lum = np.concatenate(
            (north_table['r1t_lum'][north_ind], south_table['r1t_lum'][south_ind]))
        dist = np.concatenate(
            (north_table['dist'][north_ind], south_table['dist'][south_ind]))

        mDist = np.concatenate(
            (north_table['mDist'][north_ind], south_table['mDist'][south_ind]))
        mDistErr = np.concatenate(
            (north_table['mDistErr'][north_ind], south_table['mDistErr'][south_ind]))
        sigmaP_lum = np.concatenate(
            (north_table['sigmaP_lum'][north_ind], south_table['sigmaP_lum'][south_ind]))
        ...
        Vls1 = np.concatenate(
            (north_table['Vls'][north_ind], south_table['Vls'][south_ind]))
        R_2t21 = np.concatenate(
            (north_table['R2t_lum'][north_ind], south_table['R2t_lum'][south_ind]))
        nest1 = np.concatenate(
            (north_table['nest'][north_ind], south_table['nest'][south_ind]))

        r1t_lum1 = np.concatenate(
            (north_table['r1t_lum'][north_ind], south_table['r1t_lum'][south_ind]))
        dist1 = np.concatenate(
            (north_table['dist'][north_ind], south_table['dist'][south_ind]))

        mDist1 = np.concatenate(
            (north_table['mDist'][north_ind], south_table['mDist'][south_ind]))
        mDistErr1 = np.concatenate(
            (north_table['mDistErr'][north_ind], south_table['mDistErr'][south_ind]))
        sigmaP_lum1 = np.concatenate(
            (north_table['sigmaP_lum'][north_ind], south_table['sigmaP_lum'][south_ind]))
...
\n
 ...
 - Example(s): 
    ...
    $ python %prog -f iter.2.v30.supergroup -c supergalactic -a 360 -d 0 w 30
    - This would plot north and south (Galactic) catalogs together, where SGL=0 or SGL=360
    
    $ python %prog -f iter.2.v30.supergroup -c supergalactic -p  34695
...
def main(alpha, delta, fig, ax, width=10, mode='equatorial', V_range=None, SGX=None, SGY=None, SGZ=None, Projection=None):
    ...
    if mode != "cartesian":
        ax.annotate(x0_text+'{:.4f}'.format(alpha),
                    (0.016, 0.86), xycoords='figure fraction', size=11)
        ax.annotate(y0_text+'{:.4f}'.format(delta),
                    (0.016, 0.82), xycoords='figure fraction', size=11)
    elif mode == "cartesian":
        if SGX != None and SGY != None and Projection == 1:
            ax.annotate(x0_text+'{:.4f}'.format(SGX), (0.016,
                                                       0.86), xycoords='figure fraction', size=11)
            ax.annotate(y0_text+'{:.4f}'.format(SGY), (0.016,
                                                       0.82), xycoords='figure fraction', size=11)
        elif SGX != None and SGZ != None and Projection == 2:
            ax.annotate(x0_text+'{:.4f}'.format(SGX), (0.016,
                                                       0.86), xycoords='figure fraction', size=11)
            ax.annotate(y0_text+'{:.4f}'.format(SGZ), (0.016,
                                                       0.82), xycoords='figure fraction', size=11)
        elif SGY != None and SGZ != None and Projection == 3:
            ax.annotate(x0_text+'{:.4f}'.format(SGY), (0.016,
                                                       0.86), xycoords='figure fraction', size=11)
            ax.annotate(y0_text+'{:.4f}'.format(SGZ), (0.016,
                                                       0.82), xycoords='figure fraction', size=11)
            ...
            ax.annotate(x0_text+'{:.4f}'.format(SGY), (0.016,
                                                       0.86), xycoords='figure fraction', size=11)
            ax.annotate(y0_text+'{:.4f}'.format(SGX), (0.016,
                                                       0.82), xycoords='figure fraction', size=11)
        elif SGX != None and SGZ != None and Projection == 20:
            ax.annotate(x0_text+'{:.4f}'.format(SGZ), (0.016,
                                                       0.86), xycoords='figure fraction', size=11)
            ax.annotate(y0_text+'{:.4f}'.format(SGX), (0.016,
                                                       0.82), xycoords='figure fraction', size=11)
            ...
            ax.annotate(x0_text+' 0', (0.016, 0.86),
                        xycoords='figure fraction', size=11)
            ax.annotate(y0_text+' 0'.format(SGX), (0.016, 0.82),
                        xycoords='figure fraction', size=11)

    ax.annotate("Coordinates ", (0.016, 0.73),
                xycoords='figure fraction', size=14, color='maroon')
    ...
    if mode != 'cartesian':
        ax.annotate("WCS ...", (0.016, 0.60),
                    xycoords='figure fraction', size=14, color='maroon')
        ax.annotate("Galactic", (0.016, 0.42),
                    xycoords='figure fraction', size=14, color='maroon')
        ax.annotate("Super-Galactic", (0.016, 0.29),
                    xycoords='figure fraction', size=14, color='maroon')
    elif mode == 'cartesian':
        ax.annotate("Selected Gaalxy", (0.016, 0.58),
                    xycoords='figure fraction', size=14, color='maroon')
    ...
    ra_list = np.concatenate(
        (np.arange(N-5, ra_min-5, -5), np.arange(N, ra_max+5, 5)))
    ...
    def in_motion(event):
        ...
        if event.inaxes == ax:
            ...
            if mode == "equatorial":
                ...
                gl, gb = tran((RA, DEC))
                tran = wcs.Transformation(
                    "equatorial j2000 j2000", "supergalactic")
                sgl, sgb = tran((RA, DEC))
                ...
                RA, DEC = tran((gl, gb))
                tran = wcs.Transformation(
                    "galactic j2000 j2000", "supergalactic")
                sgl, sgb = tran((gl, gb))

            elif mode == "supergalactic":
                sgl, sgb = transform_inv(alpha, delta, a, b)
                tran = wcs.Transformation(
                    "supergalactic j2000 j2000", "equatorial")
                RA, DEC = tran((sgl, sgb))
                tran = wcs.Transformation(
...
def ClusterPlot(id, ra, dec, Vls, R_2t2, r1t_lum, dist, nest, flag, parent, mode='none'):
    ...
    while gr == 5:
        ...
        if r <= 10000:
            ...
            line, = plt.plot(X, Y, '-', markersize=2,
                             color=colour, picker=False)
            line.set_dashes([8, 3])
            parent.list_line.append(
                [line, (Red, Green, Blue), colour, Vls_grp, True])
        ...
        X_grp = []
        Y_grp = []
        while i < N and (flag[i] == 3 or flag[i] == 4):
            RA0 = ra[i]
            DEC0 = dec[i]
            if flag[i] == 3:
                X_gal.append(RA0)
        ...
        if len(X_gal) > 0:
            point, = plt.plot(X_gal, Y_gal, 's', markersize=3,
                              color=colour, markeredgecolor='black')
            parent.list_point.append(
                [point, (Red, Green, Blue), colour, Vls_grp, True, False])
            # ----------------------------------------------------------------------------^ means it's NOT a galaxy
        if len(X_grp) > 0:
            point, = plt.plot(X_grp, Y_grp, 'o', markersize=5,
                              color=colour, markeredgecolor=colour)
            parent.list_point.append(
                [point, (Red, Green, Blue), colour, Vls_grp, True, True])
            # ----------------------------------------------------------------------------^ means it's a group
...
class control_help:
    ...
    def __init__(self, parent, text):
        ...
        # top means in the current frame
        ...
        T = tk.Listbox(top, height=50, width=80, yscrollcommand=scrollbar.set)
        #T = tk.Text(top, height=50, width=100)
...
class MainWin(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wm_title("skyplot2")

        menu = tk.Menu(self)
        self.config(menu=menu)
        filemenu = tk.Menu(menu)
        menu.add_cascade(label="File", menu=filemenu)
        filemenu.add_command(label="Open Catalog ...",
    ...
    def on_exit(self):
        ...
        if tkMessageBox.askyesno("Exit", "Do you really want to quit skyplot2?"):
            if self.frame.inputDialog != None:
                self.frame.inputDialog.quit()
            sys.exit()
    ...
    def show_frame(self):
        ...
        self.frame = Plot_page(self.container, self, self.filee, self.mode, self.alpha, self.delta, self.width,
                               self.pgc, self.Vmin, self.Vmax, self.SGX, self.SGY, self.SGZ, self.Projection, self.xt, self.yt, self.zt)
...
class VelDist:
    ...
    def __init__(self, parent, velocities):
        ...
        # to force it to be the same size
        top.resizable(width=False, height=False)
    ...
    def show_histogram(self, velocities, replot=False):
        ...
        self.ax.annotate("PGC: "+'{:d}'.format(pgc_code),
                         (0.15, 0.8), xycoords='figure fraction')
        self.ax.annotate(r"$Vls$"+": "+'{:d}'.format(vel)+"  " +
                         r"$(km/s)$", (0.15, 0.7), xycoords='figure fraction')
        self.ax.annotate(
            r"$\sigma_v$"+": "+'{:.0f}'.format(sigma), (0.15, 0.6), xycoords='figure fraction')

        if dist != 0:
            self.ax.annotate(r"$d$"+":  "+'{:.2f}'.format(dist)+r'$\pm$'+'{:.0f}'.format(
                dist_e*100)+'% Mpc', (0.15, 0.5), xycoords='figure fraction')
        else:
            self.ax.annotate(r"$d$"+":  ?", (0.15, 0.5),
...
class Plot_page(tk.Frame):
    ...
    def __init__(self, parent, controller, filee, mode, alpha, delta, width, pgc, Vmin, Vmax, SGX, SGY, SGZ, Projection, xt, yt, zt):
        ...
        info_bts = Button(resetax, '', color='white', hovercolor='white')
        ...
        self.ax = self.fig.add_axes(
            [0.25, 0.07, 7.2/13,  0.9])  # main plot area
        subplots_adjust(left=0.25, bottom=0.25)
        self.fig.patch.set_facecolor('lightgray')

        self.color_labels.append(self.ax.annotate(
            r"$\/\/\/\/\/\/\/\/\/\/ V_{ls} \/ < \/ 0   $", (0.88, 0.91), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$ \/\/\/\/ 0 \/ \leq \/ V_{ls} \/ < \/ 250$', (0.88, 0.91-0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$\/ 250 \/ \leq \/ V_{ls} \/ < \/ 500$', (0.88, 0.91-2*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$\/ 500 \/ \leq \/ V_{ls} \/ < \/ 750$', (0.88, 0.91-3*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$\/ 750 \/ \leq \/ V_{ls} \/ < \/ 1000$', (0.88, 0.91-4*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$1000 \/ \leq \/ V_{ls} \/ < \/ 1250$', (0.88, 0.91-5*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$1250 \/ \leq \/ V_{ls} \/ < \/ 1500$', (0.88, 0.91-6*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$1500 \/ \leq \/ V_{ls} \/ < \/ 1750$', (0.88, 0.91-7*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$1750 \/ \leq \/ V_{ls} \/ < \/ 2000$', (0.88, 0.91-8*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$2000 \/ \leq \/ V_{ls} \/ < \/ 2500$', (0.88, 0.91-9*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$2500 \/ \leq \/ V_{ls} \/ < \/ 3000$', (0.88, 0.91-10*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$3000 \/ \leq \/ V_{ls} \/ < \/ 3500$', (0.88, 0.91-11*0.05), xycoords='figure fraction', size=12, color='black'))
        self.color_labels.append(self.ax.annotate(
            r'$3500 \/ \leq \/ V_{ls}$', (0.88, 0.91-12*0.05), xycoords='figure fraction', size=12, color='black'))
        ...
        if Vmin == None:
            self.label_vmin = self.ax.annotate(
                r"$V_{min}: - $", (0.01, 0.97), xycoords='figure fraction', size=12, color='black')
            v_min.set("V.min : -")
            Velmin = -100000.
        else:
            self.label_vmin = self.ax.annotate(
                r"$V_{min}: $"+str(Vmin), (0.01, 0.97), xycoords='figure fraction', size=12, color='black')
            v_min.set("V.min : "+str(Vmin))
            Velmin = Vmin

        if Vmax == None:
            self.label_vmax = self.ax.annotate(
                r"$V_{max}: - $", (0.13, 0.97), xycoords='figure fraction', size=12, color='black')
            v_max.set("V.max : -")
            Velmax = 100000.
        else:
            self.label_vmax = self.ax.annotate(
                r"$V_{max}: $"+str(Vmax), (0.13, 0.97), xycoords='figure fraction', size=12, color='black')
                    ...
                    for i in range(len(self.browser.id_gal)):
                        if s == self.browser.id_gal[i]:
                            ...
                            for pp in range(3):
                                ...
                                self.fig.canvas.draw()
                                time.sleep(0.7)
                                self.browser.group_plot.set_visible_gal(
                                    self.browser.id_gal[i])
                                self.fig.canvas.draw()
                                time.sleep(0.7)
        ...
        b_down = tk.Button(self.win, text="v",
                           command=call_down, font=LARGE_FONT)
        b_left = tk.Button(self.win, text="<",
                           command=call_left, font=LARGE_FONT)
        b_right = tk.Button(self.win, text=">",
                            command=call_right, font=LARGE_FONT)
    ...
    def show_plot(self):
        ...
        if mode != 'cartesian':
            ...
            sgx, sgy, sgz = xyz_list(sgl, sgb, Vls, dist, mDist, flag)
        ...
        table_gal = {'pgc': id_gal, 'flag': flag_gal, 'sgl': sgl_gal, 'sgb': sgb_gal, 'gl': gl_gal, 'gb': gb_gal, 'ra': ra_gal, 'dec': dec_gal, 'Ks': Ks_gal, 'Vls': Vls_gal,
                     'R2t_lum': R_2t2_gal, 'nest': nest_gal, 'r1t_lum': r1t_lum_gal,  'dist': dist_gal,
        ...
        self.line, = plt.plot(A, B, 'o', markersize=1, color='white',
                              markeredgecolor='white', picker=5)   # color='#696969'

        self.lline = []
        for i in range(len(id_gal)):

            random.seed(id_gal[i])
            Red, Green, Blue = random.random(), random.random(), random.random()

            if flag_gal[i] == 3 or flag_gal[i] == 0:
                l, = plt.plot([A[i]], [B[i]], 'x', markersize=2.5,
                              color='white', markeredgecolor=color_table(Vls_gal[i]))
                self.lline.append(
                    [l, color_table(Vls_gal[i]), Vls_gal[i], True, False, '#696969'])
                # ---------------------------------------------------------------^ means it's a galaxy

            if flag_gal[i] == 4 or flag_gal[i] == 2:
                l, = plt.plot([A[i]], [B[i]], 'o', markersize=3.5,
                              color='white', markeredgecolor=color_table(Vls_gal[i]))
                self.lline.append(
                    [l, color_table(Vls_gal[i]), Vls_gal[i], True, True, (Red, Green, Blue)])
                # ---------------------------------------------------------------^ means it's a group
        ...
        ClusterPlot(id, A_cluster, B_cluster, Vls, R_2t2,
                    r1t_lum, dist, nest, flag, self, mode=mode)

        self.browser = PointBrowser(
            A, B, table_gal, A_cluster, B_cluster, table_all, self, mode)

        self.ax.minorticks_on()
        self.ax.tick_params(which='major', length=7, width=1.5)
        self.ax.tick_params(which='minor', length=4,
                            color='#000033', width=1.0)
        ...
        self.ax.annotate("Catalog Name:", (0.016, 0.12),
        ...
        if spl[-1] == '':
            ...
            label = spl[-1]
        self.ax.annotate(label, (0.016, 0.08),
                         xycoords='figure fraction', size=10, color='black')
        ...
        def on_click(event):
            ...
            if event.dblclick and event.button == 3:
                ...
                alpha, delta = transform_inv(
                    self.alpha, self.delta, event.xdata, event.ydata)
        ...
        for p in range(0, len(col_pallet)):
            ...
            self.color_bts.append(
                [Button(resetax, '', color=col_pallet[p], hovercolor=col_pallet[p]), True])
            ...
            if self.color_bts[p][1]:
                ...
                for obj in self.list_point:
                    if vel_limits(p, obj[3]) and obj[4]:
                        if obj[5]:
                            obj[0].set_visible(True)
                        if not obj[5] and gal_on:
                            obj[0].set_visible(True)
                for obj in self.lline:
                    if vel_limits(p, obj[2]) and obj[3]:
                        if obj[4]:
                            obj[0].set_visible(True)
                        if not obj[4] and gal_on:
                            obj[0].set_visible(True)
        ...
        self.resetax_button = Button(
            resetax, "Reset view", color=axcolor, hovercolor='navajowhite')
        ...
        ###############################
        rax0 = axes([0.85, 0.03, 0.11, 0.07])
        self.radio_gal = RadioButtons(rax0, ('On', 'Off'), active=0)
        annotate('Galaxies on/off ?', (0.86, 0.11),
                 xycoords='figure fraction', size=10, color='black')
        ...
        self.radio_gal.on_clicked(galfunc)
        #######################
        ###############################
        rax = axes([0.85, 0.14, 0.11, 0.11])
        self.radio = RadioButtons(rax, ('No', 'Yes'), active=0)
        annotate('Random Colors ?', (0.86, 0.26),
                 xycoords='figure fraction', size=10, color='black')
        ...
        def colorfunc(label):
            ...
            if label == 'Yes':
                ...
                for obj in self.list_point:
                    obj[0].set_color(obj[1])
                    if not obj[5]:
                        obj[0].set_markeredgecolor('black')
                    else:
                        obj[0].set_markeredgecolor(obj[1])

                for obj in self.lline:
                    obj[0].set_markeredgecolor(obj[5])
        ...
        self.canvas.mpl_connect(
            'button_press_event', lambda event: self.canvas._tkcanvas.focus_set())
